...
// GET '/characters' retorna: status 200  y [{id:numerico, nombre:string}]  - status: 413 y []

app.get('/characters', async (req, res) => {
    try{
        const query = 'SELECT image, name FROM characters';
...
/*
app.get('/characters/:idName', async (req, res) => {
    try{
        const query = 'SELECT * FROM characters WHERE idName=?';
        const respuesta = await qy(query,[req.params.id]);
        
        // Verifico que exista ese personaje
...
app.post('/characters', async (req, res) => {
    try{
        ...
        if (!req.body.name || !req.body.image || !req.body.movies){
            throw new Error ('Falta enviar el nombre, la imagen o las películas');
        }
